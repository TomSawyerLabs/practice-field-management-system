...
| `LAN_URL`        | _(none)_            | URL devices on the field network use to open pFMS, e.g. `http://pfms.tsl/`. When set, `/health/site` fetches it on every address the name resolves to and returns 503 if any of them serves the public-only page — the check behind uptime monitoring. |
...
| `MATCH_RECORDINGS_DIR`           | `recordings`                        | Where recorded matches and practice runs are stored (one directory per match id / run id; the practice ring buffer lives in `.practice-buffer/` underneath).                                                                                                                                                                                                                                         |
| `PRACTICE_RECORDINGS_FILE`       | `practice-recordings.json`          | Index for "record while enabled": which teams opted in, the recorded runs, and the per-team per-day share tokens. See [record while enabled](match-system.md#record-while-enabled-practice-runs).                                                                                                                                                                                                    |
| `TEAM_CONTACTS_FILE`             | `team-contacts.json`                | Team number → Slack channel or people, set on the admin page; where each team's practice-day link is posted.                                                                                                                                                                                                                                                                                         |
...
| `PUBLIC_URL`                     | _(page's own origin)_               | Address the field is reachable at from anywhere, e.g. `https://pfms.example.org`. Used for the post-match QR code and share links (`/matches/<token>`, `/practice/<token>`). The setup UI value wins.                                                                                                                                                                                                |
