...
| **Radio QoS BW Limit** | "Enable QoS BW Limit" checkbox (`qosEnabled`) and the robot's current `bandwidthUsedMbps` on its linked band | `false`. When enabled it **warns** and shows current usage; at/near the ~4 Mbps cap it says the team is hitting it (the cause of lag/dropouts). The practice field sets no limit.        |
