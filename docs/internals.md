...
| `/health/site`               | Uptime probe: 200 when the backend is up and every address of `LAN_URL` serves the internal UI, 503 otherwise. Separate from `/health` so a failing check can never unblock `update.sh`'s match guard. |
