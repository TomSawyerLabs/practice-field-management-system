...
| Some Wi-Fi devices intermittently can't load the page | Wired→wireless ARP isn't delivered (AP broadcast filtering, or a loop). `arping`/`ip neigh` from the server shows the client INCOMPLETE — not a DNS or netmask problem |
